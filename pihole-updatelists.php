    ...
    if (!empty($configSectionData['ADLISTS_URL'])) {
        ...
        if (($sth = $dbh->prepare('SELECT * FROM `adlist_by_group`'))->execute()) {
            ...
            foreach ($adlistsGroupsAll as $key => $value) {
                ...
                $tmp[$value['adlist_id']][] = $value['group_id'];
            }
            ...
            foreach ($adlists as $address) {
                ...
                if ($adlistUrl === false) {
                    ...
                    if (
                        ...
                        $config['VERBOSE'] === true && $sth->rowCount() && printAndLog('Re-added to the group with ID = ' . $absoluteGroupId . PHP_EOL);
                    }
                    ...
                    if (
                        ...
                        $config['VERBOSE'] === true && $sth->rowCount() && printAndLog('Re-added entry to the default group' . PHP_EOL);
                    }
                }
            }
                ...
                foreach ($domainlist as $domain) {
                    ...
                    if ($domainlistDomain === false) {
                        ...
                        if (
                            ...
                            $config['VERBOSE'] === true && $sth->rowCount() && printAndLog('Re-added to the default group' . PHP_EOL);
                        }
                    }
                }
